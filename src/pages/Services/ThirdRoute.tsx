...
export function ThirdRoute(props: any) {
    ...
    return (
        <ScrollView contentContainerStyle={{ paddingBottom: calcHeight(50) }} style={{ flexGrow: 0 }} >
            {props.data ? <FlatList
                data={props.data.filter((item: any) => item.service_type == props.location).filter((item: any) => item.target_client_gender == 'ALL').filter((item: any) => item.name.includes(props.search))}
                style={{
