...
export const ThirdRoute = (props: any) => {
    ...
    useEffect(() => {
        if (props.chart) {
            ...
            let tempDays = []
            for (let i = 0; i < 7; i++) {
                tempBooking.push({ value: props.chart.booking[i][Object.keys(props.chart.booking[i])[0]] })
                tempPending.push({ value: props.chart.pending[i][Object.keys(props.chart.pending[i])[0]] })
                tempRejected.push({ value: props.chart.rejected[i][Object.keys(props.chart.rejected[i])[0]] })
                tempSales.push({ value: props.chart.sales[i][Object.keys(props.chart.sales[i])[0]] })
                tempDays.push(moment(Object.keys(props.chart.booking[i])[0]).format('ddd').toUpperCase())
            }
            setDays(tempDays)
    ...
    return (
        <ScrollView
            ...
            <HomeItem color='#EBEBEB' title='Abandoned Reservations' icon={<CalendarExclam />} id={0} />
        </ScrollView>
    )
};